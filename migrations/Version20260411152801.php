 ...
 */
final class Version20260411152801 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        // investissement FKs — only if missing
        $investFks = array_map(fn($fk) => $fk->getName(), $sm->listTableForeignKeys('investissement'));
        if (!in_array('FK_B8E64E01166D1F9C', $investFks)) {
            $this->addSql('ALTER TABLE investissement ADD CONSTRAINT FK_B8E64E01166D1F9C FOREIGN KEY (project_id) REFERENCES project (id)');
        }
        if (!in_array('FK_B8E64E01A76ED395', $investFks)) {
            $this->addSql('ALTER TABLE investissement ADD CONSTRAINT FK_B8E64E01A76ED395 FOREIGN KEY (user_id) REFERENCES user (id)');
        }

        // shared_posts datetime fix
        $this->addSql('ALTER TABLE shared_posts CHANGE shared_at shared_at DATETIME NOT NULL');

        // user — drop unique indexes only if they exist, then CHANGE
        $userIndexes = array_map(fn($i) => $i->getName(), $sm->listTableIndexes('user'));
        if (in_array('UNIQ_8D93D649ABE530DA', $userIndexes)) {
            $this->addSql('DROP INDEX UNIQ_8D93D649ABE530DA ON user');
        }
        if (in_array('UNIQ_8D93D649E7927C74', $userIndexes)) {
            $this->addSql('DROP INDEX UNIQ_8D93D649E7927C74 ON user');
        }
        if (in_array('UNIQ_8D93D6497300D14B', $userIndexes)) {
